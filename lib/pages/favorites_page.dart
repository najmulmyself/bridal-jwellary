...
class FavoritesPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(centerTitle: true, title: Text('Favorites', style: Theme.of(context).textTheme.titleLarge?.copyWith(color: cs.onSurface))),
      body: SafeArea(
      ...
      child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Container(
            width: 120,
            height: 120,
            decoration: BoxDecoration(color: cs.primaryContainer, borderRadius: BorderRadius.circular(AppTokens.r24)),
            child: Icon(Icons.favorite_border, color: cs.primary, size: 56),
          ),
          const SizedBox(height: 16),
          Text('No favorites yet', style: Theme.of(context).textTheme.titleMedium?.copyWith(color: cs.onSurface)),
          const SizedBox(height: 8),
          Text('Tap the heart on any design to save it here.', style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: cs.onSurface.withValues(alpha: 0.8))),
        ]),
      ),
    );
  }
